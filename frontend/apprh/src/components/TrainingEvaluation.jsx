...
const TrainingPlansTab = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingPlan) {
        await api.put(`/ditech/training-plans/${editingPlan.id}/`, data)
        showToast('Plan de formation modifié avec succès', 'success')
      } else {
  ...
  const handleStatusChange = async (plan, action) => {
    try {
      if (action === 'activate') {
                    ...
                    <div className="action-buttons">
                      ...
                      {plan.status === 'DRAFT' && (
                        <button
                          ...
                          onClick={() => handleStatusChange(plan, 'activate')}
                          title="Activer"
                        >
                          ▶️
                        </button>
                      ...
                      {plan.status === 'ACTIVE' && (
                        <button
                          ...
                          onClick={() => handleStatusChange(plan, 'complete')}
                          title="Terminer"
                        >
                          <CheckCircle size={14} />
                        </button>
        ...
        <div className="dialog-overlay" onClick={() => setShowDialog(false)}>
          <div className="dialog dialog-large" onClick={(e) => e.stopPropagation()}>
            <div className="dialog-header">
              <h2>{editingPlan ? 'Modifier le Plan' : 'Nouveau Plan de Formation'}</h2>
...
const EvaluationsTab = () => {
  ...
  const openEditDialog = (evaluation) => {
    ...
    setFormData({
      ...
      evaluation_type: evaluation.evaluation_type || 'ANNUAL',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingEvaluation) {
        await api.put(`/ditech/evaluations/${editingEvaluation.id}/`, data)
...
const ManagerFeedbackTab = () => {
  ...
  const openViewFeedbackDialog = (evaluation) => {
    setSelectedEvaluation(evaluation)
                  ...
                  <td>
                    <div className="action-buttons">
                      {evaluation.manager_feedback && (
                        <button
                          className="btn-icon btn-primary"
                          onClick={() => openViewFeedbackDialog(evaluation)}
                      ...
                      <button
                        className="btn-icon btn-primary"
                        onClick={() => openFeedbackDialog(evaluation)}
                        title={evaluation.manager_feedback ? "Modifier feedback" : "Ajouter feedback"}
                      >
