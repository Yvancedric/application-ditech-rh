...
const JobOffersTab = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingOffer) {
        await api.put(`/ditech/job-offers/${editingOffer.id}/`, dataToSend)
...
const CandidatesTab = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingCandidate) {
        await api.put(`/ditech/candidates/${editingCandidate.id}/`, dataToSend)
        showToast('Candidat modifié avec succès', 'success')
      } else {
    ...
    <div className="candidates-tab">
      ...
      {showDialog && (
        <div className="dialog-overlay" onClick={() => setShowDialog(false)}>
          <div className="dialog dialog-large" onClick={(e) => e.stopPropagation()}>
            <div className="dialog-header">
              <h2>{editingCandidate ? 'Modifier le Candidat' : 'Nouveau Candidat'}</h2>
              <button className="btn-close" onClick={() => setShowDialog(false)}>
                <X size={20} />
              </button>
            </div>
...
const InterviewsTab = () => {
  ...
  const openEditDialog = (interview) => {
    ...
    // Log pour déboguer
    if (import.meta.env.MODE === 'development') {
      console.log('Interview à modifier:', interview)
      console.log('Interviewer ID:', interview.interviewer, interview.interviewer_detail)
    }
    
    // Extraire l'ID de l'interviewer (peut être un nombre ou un objet)
    let interviewerId = ''
    ...
    } else if (interview.interviewer_detail?.id) {
      interviewerId = interview.interviewer_detail.id
    }
    
    // S'assurer que interviewerId est une string ou une chaîne vide
    const interviewerIdString = interviewerId ? String(interviewerId) : ''
    
    setFormData({
      candidate: interview.candidate?.id || interview.candidate || '',
      interviewer: interviewerIdString,
      scheduled_date: dateStr,
      scheduled_time: timeStr,
      interview_type: interview.interview_type || 'ONSITE',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingInterview) {
        await api.put(`/ditech/interviews/${editingInterview.id}/`, dataToSend)
              ...
              {currentCandidates.map(candidate => {
                ...
                return (
                  <tr key={candidate.id}>
                    ...
                    <td>
                      <div className="action-buttons">
                        {candidate.status === 'FINAL_REVIEW' && (
                          <button
                            className="btn-icon btn-primary"
                            onClick={() => handleStatusUpdate(candidate, 'OFFER_SENT')}
                        ...
                        {candidate.status === 'OFFER_SENT' && (
                          <>
                            <button
                              ...
                              onClick={() => handleStatusUpdate(candidate, 'OFFER_ACCEPTED')}
                        ...
                        {(candidate.status === 'HIRED' || candidate.status === 'REJECTED') && (
                          <span className="status-final" title={`Statut final: ${getStatusDisplay(candidate.status)}`}>
                            {candidate.status === 'HIRED' ? <PartyPopper size={16} /> : <XCircle size={16} />}
                          </span>
                        )}
                      </div>
                    </td>
