...
const PayslipsTab = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingPayslip) {
        await api.put(`/ditech/payslips/${editingPayslip.id}/`, dataToSend)
        showToast('Fiche de paie modifiée avec succès', 'success')
      } else {
                  ...
                  <td>
                    <div className="action-buttons">
                      <button
                        ...
                        onClick={() => openEditDialog(payslip)}
                        title="Modifier"
                      >
                        <Pencil size={16} />
                      </button>
        ...
        <div className="dialog-overlay" onClick={() => setShowDialog(false)}>
          <div className="dialog dialog-large" onClick={(e) => e.stopPropagation()}>
            <div className="dialog-header">
              <h2>{editingPayslip ? 'Modifier la Fiche de Paie' : 'Nouvelle Fiche de Paie'}</h2>
...
const BonusesTab = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingBonus) {
        await api.put(`/ditech/payslip-bonuses/${editingBonus.id}/`, dataToSend)
    ...
    <div className="bonuses-tab">
      ...
      {showDialog && (
        <div className="dialog-overlay" onClick={() => setShowDialog(false)}>
          <div className="dialog dialog-large" onClick={(e) => e.stopPropagation()}>
            <div className="dialog-header">
              <h2>{editingBonus ? 'Modifier la Prime' : 'Nouvelle Prime'}</h2>
              <button className="btn-close" onClick={() => setShowDialog(false)}>
                <X size={20} />
              </button>
            </div>
...
const RetentionsTab = () => {
  ...
  const openEditDialog = (retention) => {
    setEditingRetention(retention)
    setFormData({
      payslip: retention.payslip,
      deduction_type: retention.deduction_type,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingRetention) {
        await api.put(`/ditech/payslip-deductions/${editingRetention.id}/`, dataToSend)
        showToast('Retenue modifiée avec succès', 'success')
      } else {
  ...
  const handleDelete = async (retention) => {
    ...
    <div className="retentions-tab">
                ...
                <tr key={retention.id}>
                  ...
                  <td>
                    <div className="action-buttons">
                      ...
                      <button
                        className="btn-icon btn-delete"
                        onClick={() => handleDelete(retention)}
                        title="Supprimer"
                      >
                        <Trash2 size={16} />
                      </button>
      ...
      {showDialog && (
        <div className="dialog-overlay" onClick={() => setShowDialog(false)}>
          <div className="dialog dialog-large" onClick={(e) => e.stopPropagation()}>
            <div className="dialog-header">
              <h2>{editingRetention ? 'Modifier la Retenue' : 'Nouvelle Retenue'}</h2>
              <button className="btn-close" onClick={() => setShowDialog(false)}>
                <X size={20} />
              </button>
            </div>
            <form onSubmit={handleSubmit} className="dialog-form">
              ...
              <div className="dialog-actions">
                ...
                <button type="submit" className="btn-primary">
                  {editingRetention ? 'Modifier' : 'Ajouter'}
                </button>
              </div>
            </form>
          </div>
...
const DeductionsTab = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingDeduction) {
        await api.put(`/ditech/payslip-deductions/${editingDeduction.id}/`, dataToSend)
        showToast('Déduction modifiée avec succès', 'success')
      } else {
        await api.post('/ditech/payslip-deductions/', dataToSend)
    ...
    <div className="deductions-tab">
      ...
      {showDialog && (
        <div className="dialog-overlay" onClick={() => setShowDialog(false)}>
          <div className="dialog dialog-large" onClick={(e) => e.stopPropagation()}>
            <div className="dialog-header">
              <h2>{editingDeduction ? 'Modifier la Déduction' : 'Nouvelle Déduction'}</h2>
              <button className="btn-close" onClick={() => setShowDialog(false)}>
                <X size={20} />
              </button>
            </div>
