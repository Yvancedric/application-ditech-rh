...
const PointageTab = () => {
  ...
  const openEditDialog = (tracking) => {
    ...
    setFormData({
      employee: tracking.employee?.id || tracking.employee || '',
      date: tracking.date ? tracking.date.split('T')[0] : '',
      check_in_time: tracking.check_in_time ? tracking.check_in_time.slice(11, 16) : '',
      check_out_time: tracking.check_out_time ? tracking.check_out_time.slice(11, 16) : '',
      check_in_method: tracking.check_in_method || 'MANUAL',
      badge_id: tracking.badge_id || '',
      expected_check_in: tracking.expected_check_in || '09:00',
      expected_check_out: tracking.expected_check_out || '18:00',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingTracking) {
        await api.put(`/ditech/presence-tracking/${editingTracking.id}/`, dataToSend)
        showToast('Pointage modifié avec succès')
      } else {
        ...
        <table className="data-table">
          ...
          <tbody>
            {currentTrackings.length === 0 ? (
              <tr>
                <td colSpan="10" className="no-data">Aucun pointage trouvé</td>
              </tr>
            ) : (
                  ...
                  <td>
                    <div className="action-buttons">
                      ...
                      <button
                        ...
                        onClick={() => setDeleteConfirm(tracking)}
                        title="Supprimer"
                      >
                        <Trash2 size={16} />
                      </button>
        ...
        <div className="dialog-overlay" onClick={() => setShowDialog(false)}>
          <div className="dialog dialog-large" onClick={(e) => e.stopPropagation()}>
            <div className="dialog-header">
              <h2>{editingTracking ? 'Modifier le Pointage' : 'Nouveau Pointage'}</h2>
...
const CongesTab = () => {
  ...
  const openEditDialog = (request) => {
    ...
    setFormData({
      employee: request.employee?.id || request.employee || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingRequest) {
        await api.put(`/ditech/leave-requests/${editingRequest.id}/`, dataToSend)
        showToast('Demande de congé modifiée avec succès')
      } else {
        ...
        <table className="data-table">
          ...
          <tbody>
            {currentRequests.length === 0 ? (
              <tr>
                <td colSpan="8" className="no-data">Aucune demande de congé trouvée</td>
              </tr>
            ) : (
              currentRequests.map(request => {
                const balance = getEmployeeBalance(request.employee?.id || request.employee)
                return (
                  <tr key={request.id}>
                      ...
                      <div className="action-buttons">
                        {request.status === 'PENDING' && (
                          <>
                            <button
                              ...
                              onClick={() => openApprovalDialog(request, 'manager_approve')}
                            ...
                            <button
                              ...
                              onClick={() => openApprovalDialog(request, 'manager_reject')}
                              title="Rejeter (Manager)"
                            >
                              <XCircle size={16} />
                            </button>
                          </>
                        ...
                        {request.status === 'MANAGER_APPROVED' && (
                          <>
                            <button
                              className="btn-icon btn-approve"
                              onClick={() => openApprovalDialog(request, 'rh_approve')}
                              title="Approuver (RH)"
                            >
                              <CheckCircle2 size={16} /> RH
                            </button>
                            <button
                              className="btn-icon btn-reject"
                              onClick={() => openApprovalDialog(request, 'rh_reject')}
                              title="Rejeter (RH)"
                            >
                              <XCircle size={16} /> RH
                            </button>
                        ...
                        {request.status !== 'RH_APPROVED' && request.status !== 'CANCELLED' && (
                          <button
                            className="btn-icon btn-edit"
                        ...
                        {balance && (
                          <button
                            className="btn-icon btn-balance"
                            onClick={() => {
